    ...
    <section class="w-full bg-[var(--scf-bg)] px-6 py-8 md:px-[60px]">
      <div class="page-shell grid items-start gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <div class="rounded-[26px] bg-white p-9 lg:sticky lg:top-24">
          ...
          <h1 class="display-font mb-6 text-4xl font-extrabold text-[var(--scf-ink)]">
            {{ cat.name }}
          </h1>
          ...
          <div class="mb-7 flex flex-col gap-4">
            <div
              v-if="healthSummary"
              class="flex justify-between gap-4 border-b border-[var(--scf-line)] pb-3.5"
            >
            ...
            <div v-if="cat.trappingDate" class="flex justify-between gap-4">
              <span class="text-sm font-semibold text-[var(--scf-muted)]">{{
          ...
          <Button
            as="a"
            :href="buildMailtoLink('Question à propos de ' + cat.name)"
